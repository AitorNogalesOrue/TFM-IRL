...
def main():
    ...
    learner = app.calc_feature_expectation(feature_num, gamma, q_table, demonstrations, env)
    learner = np.matrix([learner])
    
    expert = app.expert_feature_expectation(feature_num, gamma, demonstrations, env)
    expert = np.matrix([expert])
    
    w, status = app.QP_optimizer(feature_num, learner, expert)
    ...
    for episode in range(60000):
        ...
        if episode % 5000 == 0:
            ...
            status = "infeasible"
            temp_learner = app.calc_feature_expectation(feature_num, gamma, q_table, demonstrations, env)
            learner = app.add_feature_expectation(learner, temp_learner)
            
            while status=="infeasible":
                w, status = app.QP_optimizer(feature_num, learner, expert)
                if status=="infeasible":
                    learner = app.subtract_feature_expectation(learner)
