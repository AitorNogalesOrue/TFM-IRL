...
class FeatureEstimate:
    ...
    def get_features(self, state):
        ...
        env_distance_0 = (env_high[0] - env_low[0]) / (self.feature_num - 1)
        env_distance_2 = (env_high[2] - env_low[2]) / (self.feature_num - 1)
        for i in range(int(self.feature_num/2)):
            # position
            self.feature[i] = self.gaussian_function(state[0]*10, 
                                    env_low[0]*10 + i * env_distance_0*10)
            # angle
            self.feature[i+int(self.feature_num/2)] = self.gaussian_function(state[2]*10, 
                                                        env_low[2]*10 + i * env_distance_2*10)
